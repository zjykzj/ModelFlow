...
⚙️  预处理配置：CenterCrop -> Resize(224x224) -> Normalize
--------------------------------------------------
   已处理 100 / 100 ...
--------------------------------------------------
...
$$ \text{文件大小} = \text{宽} \times \text{高} \times \text{通道数} \times \text{单元素字节数} $$

**代入数值验证：**

$$ 602,112 \text{ Bytes} = 224 \times 224 \times 3 \times 4 \ (\text{Float32}) $$
...
def main():
    ...
    parser.add_argument("--crop_size", type=int, default=224, help="目标高度 (默认 224)")
    ...
    print(f"⚙️  预处理配置：CenterCrop -> Resize({args.crop_size}x{args.crop_size}) -> Normalize")
    print("-" * 50)

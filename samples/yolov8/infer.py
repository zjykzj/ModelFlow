...
def parse_opt() -> argparse.Namespace:
    ...
    parser.add_argument(
        "model",
        type=str,
        help="Path to model file (e.g., yolov8s.onnx)"
    )
    parser.add_argument(
        "input",
        type=str,
...
def main():
    ...
    if args.backend == "onnxruntime":
        if args.processor == "torch":
            try:
                from yolov8_runtime_w_torch import YOLOv8Runtime
            except ImportError:
                raise ImportError(f"PyTorch processor selected, but YOLOv8Runtime is not available.")
        else:
            try:
                from yolov8_runtime_w_numpy import YOLOv8Runtime
            except ImportError:
                raise ImportError(f"Numpy processor selected, but YOLOv8Runtime is not available.")

        ModelClass = YOLOv8Runtime

...
cv::Mat imagePreprocess(std::vector<int64_t> &inputDims) {
...
int main(int argc, char *argv[]) {
    ...
    cv::Mat preprocessedImage = imagePreprocess(input_node_dims);
    input_tensor_values.assign(preprocessedImage.begin<float>(),
...
//    assert(abs(floatarr[0] - 0.014135006) < 1e-6);
    ...
    int num_classes = 10;
    assert(num_classes == output_node_dims[1]);
    ...
    int64_t result_ = std::distance(output_tensor_values.begin(),
                                    std::max_element(output_tensor_values.begin(),
                                                     output_tensor_values.end()));
    printf("max index is %ld\n", result_);

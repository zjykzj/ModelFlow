...
private:
    ...
    // 保存分类概率
    std::vector<float> _output_results;
...
#undef TBLOB_TOP_RESULT
    ...
    void getOutputs(unsigned int n, InferenceEngine::Blob &input, std::vector<float> &output) {
...
public:
    explicit ClassificationResultT(InferenceEngine::Blob::Ptr output_blob, std::vector<strType> image_names = {},
        ...
        getOutputs(_nTop, *_outBlob, _output_results);
    }

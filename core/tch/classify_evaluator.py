...
class EvalEvaluator:
    ...
    def run(self):
        ...
        """
        num_classes = len(self.dataset.classes_list)
        ...
        for idx in tqdm(range(len(self.dataset))):
            ...
            #     logger.info(f"Evaluate images {idx + 1}/{total}")

            img, cls_idx, class_name, img_name, img_path = self.dataset.__getitem__(idx)

            # 推理开始计时
            t1 = time.time()

            input_data = self.transform(img).to(self.device)

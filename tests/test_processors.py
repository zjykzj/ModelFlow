...
覆盖: classify, detect, segment, semantic_seg, multimodal 的 pre/post processors
...
from modelflow.processors.multimodal import CLIPImagePreprocessor, CLIPPostprocessor
...
class TestSemanticSegProcessor:
    ...
    def test_postprocess_argmax(self):
        postproc = SemanticSegPostprocessor(apply_colormap=False)
    ...
    def test_postprocess_colormap(self):
        postproc = SemanticSegPostprocessor(apply_colormap=True, colormap="voc")
        logits = np.random.randn(1, 21, 224, 224).astype(np.float32)
        result = postproc([logits])

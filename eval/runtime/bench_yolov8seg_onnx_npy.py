...
"""
@Time    : 2026/2/22 16:22
@File    : bench_yolov8-seg_onnx_npy.py
@Author  : zj
@Description: 
"""
...
class ONNXSegmentModel:
    ...
    def __init__(self, model_path, class_list, half=False):
        ...
        self.stride = getattr(self.session, 'stride', 32)
    ...
    def __call__(self, data):
        outputs = self.session.run(self.output_names, {self.input_names[0]: data})
        return outputs
...
if __name__ == '__main__':
    with Profile(name="evaluating") as stage_profiler:
        ...
        model = ONNXSegmentModel(model_path, class_list)

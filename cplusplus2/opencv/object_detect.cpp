...
cv::Rect DetectImg(cv::Mat &bgr) {
  ...
  cv::Mat opening;
  cv::Mat kernel =
      cv::getStructuringElement(cv::MorphShapes::MORPH_RECT, cv::Size(10, 10));
  cv::morphologyEx(thresh, opening, cv::MorphTypes::MORPH_CLOSE, kernel,
                   cv::Point(-1, -1), 6);
  cv::morphologyEx(opening, opening, cv::MorphTypes::MORPH_OPEN, kernel,
                   cv::Point(-1, -1), 3);

  std::vector<std::vector<cv::Point>> contours;
  std::vector<cv::Vec4i> hierarchy;
  cv::findContours(opening, contours, hierarchy,
...
int main(int argc, char *argv[]) {
  std::string img_path = "../asset/demo.jpg";

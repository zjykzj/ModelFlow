...
//    auto result = cv::dnn::readNet("config_files/yolov5s.onnx");
	auto result = cv::dnn::readNet("../../../../assets/yolov5n.onnx");
	if (is_cuda) {
...
class YOLODetector {
 public:
  ...
  int Detect(const cv::Mat &image, const std::vector<std::string> &class_names, std::vector<Detection> &output);
...
int DrawImage(cv::Mat &image, std::vector<Detection> output, const std::vector<std::string> &class_list) {
